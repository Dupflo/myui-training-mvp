...
type SidebarContext = {
  ...
  toggleSidebar: () => void
  fullSidebar?: boolean
  setFullSidebar: (open: boolean) => void
}
...
>(
  (
    ...
    const [_fullSidebar, _setFullSidebar] = React.useState(fullSidebar)
    ...
    const contextValue = React.useMemo<SidebarContext>(
      () => ({
        ...
        setOpenMobile,
        toggleSidebar,
        fullSidebar: _fullSidebar,
        setFullSidebar: _setFullSidebar,
      }),
...
const InnerSidebar = React.forwardRef<
  HTMLDivElement,
  React.ComponentProps<"div">
>(({ className, ...props }, ref) => {
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex min-h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-5 [&>svg]:shrink-0",
        "group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 gap-2",
        className
      )}
      {...props}
    />
